...
function toTargetsFromCsv(csvText) {
    ...
    const hasHeader = header.includes("label") || header.includes("destination") || header.includes("protocol") || header.includes("role");
...
(async () => {
    // Register as a widget (shares permissions with the parent plugin)
    ...
    // ✅ Filter on each character entered
    ...
    dialBtn.addEventListener("click", async () => {
        ...
        // Keep your known-good behavior:
        // - protocol defaults to auto
        // - avoid sending explicit protocol when destination already includes a scheme
        // - remote_display_name + text set for labeling
        const dialArgs = {
            destination,
            role,
            protocol: "auto",
            ...
            text: displayName,
            // intentionally NOT sending source_display_name (you found defaults can be better)
        };
